...
MODULE_ORDER=(browsers spotify vlc nvim vscode zed ghostty tmux zoxide p10k node uv claude codex pi caveman skills slack github docker hf ssh-keygen fonts karabiner aldente betterdisplay raycast bitwarden rectangle statusline)

module_group() {
    case "$1" in
        betterdisplay|aldente|raycast|bitwarden|rectangle) echo "Utilities" ;;
